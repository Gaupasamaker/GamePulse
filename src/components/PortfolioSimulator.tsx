    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="terminal-card p-6 bg-emerald-500/10 border-emerald-500/30">
                    <h3 className="text-xs font-mono text-emerald-400 mb-2">{t('buying_power')}</h3>
                    <div className="text-3xl font-bold text-foreground-app font-mono flex items-center gap-2">
                        <DollarSign size={24} className="text-emerald-500" />
                        {balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                </div>

                <div className="terminal-card p-6 bg-blue-500/10 border-blue-500/30">
                    ...
                    <div className="text-[10px] text-muted-foreground-app font-mono mt-1">
                ...
                <div className="terminal-card p-6">
                    <h3 className="text-xs font-mono text-muted-foreground-app mb-2">{t('gain_loss')}</h3>
                    <div className={cn(
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 terminal-card p-6 h-fit">
                    <h3 className="text-sm font-mono font-bold text-foreground-app mb-4 flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] text-muted-foreground-app font-mono uppercase">{t('ticker')}</label>
                            <select
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] text-muted-foreground-app font-mono uppercase">{t('shares')}</label>
                            <input
                                type="number"
                                className="terminal-input w-full"
                                placeholder="0"
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] text-muted-foreground-app font-mono uppercase">{t('avg_cost')}</label>
                            <input
                                type="number"
                                className="terminal-input w-full"
                                placeholder="0.00"
                ...
                <div className="lg:col-span-2 terminal-card overflow-hidden">
                    <div className="p-4 border-b border-border-app bg-secondary-app flex justify-between items-center">
                        ...
                        {user ? (
                            ...
                            <div className="text-[10px] text-muted-foreground-app font-mono flex items-center gap-1">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left font-mono text-xs">
                            <thead>
                                <tr className="bg-secondary-app/50 border-b border-border-app">
                                    <th className="px-4 py-3 text-muted-foreground-app">{t('ticker')}</th>
                                    <th className="px-4 py-3 text-right text-muted-foreground-app">{t('shares')}</th>
                                    <th className="px-4 py-3 text-right text-muted-foreground-app">{t('avg_cost')}</th>
                                    <th className="px-4 py-3 text-right text-muted-foreground-app">{t('price')}</th>
                                    <th className="px-4 py-3 text-right text-muted-foreground-app">{t('market_value')}</th>
                                    <th className="px-4 py-3 text-right text-muted-foreground-app">{t('roi')}</th>
                                    <th className="px-4 py-3 text-center text-muted-foreground-app">Action</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-border-app/50">
                                {positions.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="px-4 py-8 text-center text-muted-foreground-app italic">
                                            NO POSITIONS OPENED
                                        </td>
                                    </tr>
                                ) : (
                                    positions.map(p => {
                                        ...
                                        return (
                                            <tr key={p.id} className="hover:bg-muted-app/30">
                                                <td className="px-4 py-3 font-bold text-foreground-app">{p.ticker}</td>
                                                <td className="px-4 py-3 text-right text-muted-foreground-app">{p.shares}</td>
                                                <td className="px-4 py-3 text-right text-muted-foreground-app">${p.avgCost.toFixed(2)}</td>
                                                <td className="px-4 py-3 text-right text-foreground-app">
                                                    {loadingQuotes ? <span className="animate-pulse">...</span> : `$${price.toFixed(2)}`}
                                                </td>
                                                <td className="px-4 py-3 text-right font-bold text-foreground-app">
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <button
                                                        ...
                                                        className="text-muted-foreground-app hover:text-rose-500 transition-colors"
                                                    >
                                                        <Trash2 size={14} />
                                                    </button>
                                                </td>
