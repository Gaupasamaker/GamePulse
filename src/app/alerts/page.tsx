            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-1 flex flex-col gap-4">
                    <div className="terminal-card p-4 flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] text-muted-foreground-app font-mono uppercase">{t('type')}</label>
                            <div className="flex bg-secondary-app rounded p-1 border border-border-app">
                                <button
                                    onClick={() => setNewType('price')}
                                    className={`flex-1 py-1 text-[10px] font-mono rounded ${newType === 'price' ? 'bg-primary-app text-primary-foreground-app' : 'text-muted-foreground-app'}`}
                                >
                                    PRICE %
                                </button>
                                <button
                                    onClick={() => setNewType('keyword')}
                                    className={`flex-1 py-1 text-[10px] font-mono rounded ${newType === 'keyword' ? 'bg-primary-app text-primary-foreground-app' : 'text-muted-foreground-app'}`}
                                >
                                    KEYWORD
                                </button>
                            </div>
                        ...
                        <div className="flex flex-col gap-1">
                            <label className="text-[10px] text-muted-foreground-app font-mono uppercase">
                    ...
                    <div className="bg-primary-app/5 border border-primary-app/10 p-4 rounded-md">
                        <h3 className="text-[10px] font-mono text-primary-app flex items-center gap-2 mb-2">
                            <Info size={12} /> MVP_LIMITATION
                        </h3>
                        <p className="text-[10px] text-muted-foreground-app font-mono leading-normal">
                ...
                <div className="md:col-span-2 flex flex-col gap-4">
                    <h2 className="text-[10px] font-mono font-bold text-muted-foreground-app uppercase flex items-center gap-2">
                        <Bell size={14} /> {t('active_alerts')} ({alerts.length})
                    </h2>

                    <div className="flex flex-col gap-3">
                        {alerts.length === 0 ? (
                            <div className="terminal-card border-dashed p-12 text-center text-muted-foreground-app font-mono text-xs">
                            ...
                            alerts.map((alert) => (
                                <div key={alert.id} className={`terminal-card p-4 flex items-center justify-between border-l-4 ${alert.active ? 'border-l-emerald-500' : 'border-l-gray-600'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className={`p-2 rounded-full ${alert.active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-secondary-app text-muted-foreground-app'}`}>
                                            <Bell size={18} />
                                        </div>
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-2">
                                                <span className="text-foreground-app font-bold font-mono">{alert.ticker}</span>
                                                <span className="text-[10px] text-muted-foreground-app font-mono uppercase">
                                                    {alert.type === 'price' ? 'CHG_PERCENT' : 'NEWS_KEYWORD'}
                                                </span>
                                            </div>
                                            <span className="text-xs text-primary-app font-mono">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={`text-[10px] font-mono px-3 py-1 rounded transition-colors ${alert.active ? 'bg-background-app text-emerald-400 border border-emerald-500/20 hover:border-emerald-500' : 'bg-background-app text-muted-foreground-app border border-border-app hover:border-muted-foreground-app'}`}
                                        >
                                            {alert.active ? 'ENABLED' : 'DISABLED'}
                                        </button>
                                        <button
                                            onClick={() => deleteAlert(alert.id)}
                                            className="p-2 text-muted-foreground-app hover:text-rose-500 transition-colors"
                                        >
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
